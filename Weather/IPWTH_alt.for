...
!-----------------------------------------------------------------------
      IF (INDEX('FQ',RNMODE) <= 0 .OR. RUN. EQ. 1) THEN
         ...
         call csminp%get('*FILES','PATHWTW',PATHWTW)
...
!         and the name is read from Simulation Options.
        IF (RNMODE .EQ. 'Y') THEN
          SELECT CASE (MEWTH)
            CASE ('M')
            CASE ('G')
               call csminp%get('*FILES','FILEWG',FILEWG)
               call csminp%get('*FILES','PATHWTG',PATHWTG)
            CASE ('S','W')
               call csminp%get('*FILES','FILEWC',FILEWC)
               call csminp%get('*FILES','PATHWTC',PATHWTC)
          END SELECT
        ELSE
