...
!      INCLUDE 'COMSWI.blk'
      ...
      CHARACTER*5 SLTX
      CHARACTER*5 NCODE
      CHARACTER*5 HSTG(NAPPL),HCOM(NAPPL),HSIZ(NAPPL),IRRCOD(NAPPL)
      CHARACTER*5 RESCOD(NAPPL)
      CHARACTER*5 FERCOD(NAPPL),IFTYPE(NAPPL)
      CHARACTER*8 EXPER
      CHARACTER*16 CROPD
      CHARACTER*1 ISWWAT,ISWNIT,ISWSYM,ISWPHO,ISWDIS,MEWTH,MESIC
      CHARACTER*1 MEEVP,MEPHO,IPLTI,IIRRI,IFERI,IRESI,IHARI
      CHARACTER*1 ISWCHE,ISWTIL,MEHYD,MESOM
      CHARACTER*1 IDETO,IDETS,IDETG,IDETC,IDETW,IDETN,IDETP,IDETD,IOX
      CHARACTER*1 IDETL
      CHARACTER*2 CG
      CHARACTER*12 FILEC,FILEE,FILEG,FILES,FILEW
      CHARACTER*12 OUTO
      CHARACTER*102 DSSATP
      CHARACTER*80 PATHSL,PATHWT,PATHCR,PATHGE,PATHEC
      CHARACTER*5 SMHB,SMPX,SMKE
      CHARACTER*10 PEDON,SLNO

      INTEGER NSWITCH
      INTEGER YRSIM,YRPLT,RSEED1
      INTEGER PWDINF,PWDINL,HDLAY,HLATE,NHAR,HDATE(NAPPL)
      INTEGER NEV,WMDATE(NAPPL),NIRR,IDLAPL(NAPPL),RESDAY(NAPPL)
      INTEGER FDAY(NAPPL),NFERT,YEAR,NARES,NAPW
      INTEGER NCHEM,NTIL,CDATE(NAPPL),TDATE(NAPPL)

      REAL PLTPOP,ROWSPC,AZIR,SDEPTH,SDWTPL
      REAL SDAGE,ATEMP,SWPLTL,SWPLTH
      REAL SWPLTD,PTX,PTTN,DSOIL,THETAC,AIRAMT,EFFIRR
      REAL DSOILN,SOILNC,SOILNX,HPP,HRP
      REAL RESK(NAPPL),ANFER(NAPPL)
      REAL EFFIRX,AMT(NAPPL),RESN(NAPPL),RESP(NAPPL)
      REAL RESIDUE(NAPPL),RINP(NAPPL),DEPRES(NAPPL),DFERT(NAPPL)
      REAL TOTNAP
      REAL RESAMT,TOTAPW,WTHADJ(2,8)
      REAL SLOPE,XCRD,YCRD,ELEV
      ...
      REAL EXTP(NL)
      REAL SLNF,SLPF,DS(NL),ADCOEF(NL)
      REAL PHKCL(NL)
      REAL EXK(NL)
...
C=======================================================================
     ...
     &           CDATE,TDATE,YRIC,NTIL,NCHEM,NEV,WMDATE)
      ...
      INTEGER HLATE,PWDINF,PWDINL,NHAR,HDATE(NAPPL)
      INTEGER IDAYIC,YRIC
